...
export class RoleAssignmentsForm extends SearchComponent<Role, RoleSM, HistoryProps, SearchState<Role>> {
  ...
  init(s: RoleSM, auto: boolean) {
    Promise.all([
      ...
      this.setState({userTypeList, ctrlStatusList}, () => this.load(s, auto));
    }).catch(this.handleError);
  }

  edit = (e, id: string, cId: string) => {
    e.preventDefault();
    this.props.history.push(`role-assignment/edit/${id}/${cId}`);
  }

  approve = (e, role: Role) => {
    e.preventDefault();
    this.props.history.push(`role-assignment/approve/${role.roleId}/${role.cId}`);
  }
